 ...
 * Desarrollado con Vanilla JS y Chart.js (100% Frontend)
 */

// 1. ESTADO GLOBAL DE LA APLICACIÓN
let appState = {
    projectionData: null,          // Datos demográficos locales
...
async function initApp() {
    ...
    // Generar datos demográficos localmente
    generateLocalProjectionData();
    
    // Intentar cargar mapa SVG externo y caer en inline como fallback
    await loadSvgMap();
...
// Generación local de datos utilizando la ecuación exponencial de Malthus P(t) = P0 * e^(k*t)
function generateLocalProjectionData() {
    const years = Array.from({length: 23}, (_, i) => 2018 + i); // 2018 a 2040
    ...
    const response_data = {
        ...
        national: {
            ...
            projection: Array(years.length).fill(0),
            k_avg: 0.00078  // Promedio ponderado indicativo aproximado
        }
    };

    // Calcular proyecciones regionales
    for (let key in REGIONS_CONFIG) {
        const config = REGIONS_CONFIG[key];
        const proj = [];
        for (let i = 0; i < years.length; i++) {
            const t = years[i] - 2018;
            // Ecuación Malthusiana: P(t) = P0 * e^(k * t)
            const p_t = config.p0 * Math.exp(config.k * t);
            const p_round = Math.round(p_t);
            proj.push(p_round);
            
            // La población nacional es la suma de las proyecciones de las seis regiones
            response_data.national.projection[i] += p_t;
        }
    ...
    // Redondear proyección nacional consolidada
    response_data.national.projection = response_data.national.projection.map(Math.round);

    appState.projectionData = response_data;
}

// Cargar el mapa SVG desde un archivo externo dinámicamente
async function loadSvgMap() {
    const container = document.querySelector('.svg-map-wrapper');
    if (!container) return;

    try {
        // Intentar leer el mapa estático del directorio assets/
        const response = await fetch('assets/thailand-regions.svg');
        if (!response.ok) {
            throw new Error(`Código de estado HTTP: ${response.status}`);
        }
        const svgContent = await response.text();
        
        // Cargar SVG en el DOM
        container.innerHTML = svgContent;
        console.log("Mapa SVG cargado dinámicamente desde assets/thailand-regions.svg");
    } catch (err) {
        // En caso de error de CORS al abrir localmente por file://, el HTML mantiene el mapa inline
        console.warn("No se pudo cargar el mapa SVG externo (CORS/Entorno local). Se mantiene el mapa inline del HTML.", err);
    } finally {
        // Enlazar eventos del mapa interactivo
...
function updateSolverSelectSync() {
    ...
    const selectCalc = document.getElementById('calc-region');
    if (select) {
        select.value = appState.activeRegionKey;
    }
    if (selectCalc) {
        selectCalc.value = appState.activeRegionKey;
    }
}
...
// Renderizar la tabla de proyecciones anuales para la región activa
function renderProjectionTable() {
    const tableBody = document.getElementById('projection-table-body');
    const tableTitle = document.getElementById('table-main-title');
    if (!tableBody || !appState.projectionData) return;

    const isNational = appState.activeRegionKey === 'Total';
    const name = isNational ? 'Tailandia (Total Nacional)' : REGION_NAMES[appState.activeRegionKey];
    tableTitle.textContent = `Tabla de Datos de Proyección: ${name}`;

    const years = appState.projectionData.years;
    const proj = isNational ? appState.projectionData.national.projection : appState.projectionData.regions[appState.activeRegionKey].projection;
    const k = isNational ? appState.projectionData.national.k_avg : appState.projectionData.regions[appState.activeRegionKey].k;
    ...
    for (let i = 0; i < years.length; i++) {
        ...
        let diffClass = '';
        if (i > 0) {
            const diff = pop - proj[i - 1];
            diffText = (diff >= 0 ? '+' : '') + diff.toLocaleString('es-ES');
            diffClass = diff < 0 ? 'text-negative' : 'text-positive';
        }

        let trendText = 'Creciente';
        let trendClass = 'trend-badge grow';
        if (year === 2018) {
            trendText = 'Año Base';
            trendClass = 'trend-badge';
        } else if (k < 0) {
            trendText = 'Decreciente';
            trendClass = 'trend-badge decline';
        }

        html += `
            <tr style="border-bottom: 1px solid var(--border-color); transition: background-color 0.2s;">
                <td style="padding: 0.6rem 0.75rem; font-weight: 600;">${year}</td>
                <td style="padding: 0.6rem 0.75rem;" class="font-numeric">${pop.toLocaleString('es-ES')} hab.</td>
                <td style="padding: 0.6rem 0.75rem;" class="font-numeric ${diffClass}">${diffText}</td>
                <td style="padding: 0.6rem 0.75rem;"><span class="${trendClass}">${trendText}</span></td>
            </tr>
        `;
    }
...
// Sincronizar el marcador en el gráfico con la línea del slider temporal
...
// ==========================================================================
// 10. SOLVER INTERACTIVO (FORMULARIO MALTHUS 100% LOCAL)
// ==========================================================================
function setupSolverForm() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        // Activar spinner interactivo corto
        spinner.classList.remove('hidden');
        btn.disabled = true;

        setTimeout(() => {
            solvePopulationLocal(regionKey, parseFloat(targetVal));
...
function solvePopulationLocal(regionKey, target) {
    const data = { success: false, steps: [] };
    
    if (regionKey === 'Total') {
        const p0_total = 71376079;
        
        if (target < p0_total) {
            data.error = `La población nacional objetivo (${target.toLocaleString('es-ES')} hab.) es menor a la población base de 2018 (${p0_total.toLocaleString('es-ES')} hab.). Debido al crecimiento neto del modelo, esta cifra pertenece al pasado.`;
        } else {
            // Método de bisección local rápido en el intervalo [0, 200] años
            const regions = appState.projectionData.regions;
            let low = 0.0, high = 200.0;
            
            // Comprobar límite superior (máximo 200 años en el futuro)
            let f_high = 0;
            for (let r in regions) {
                f_high += regions[r].p0 * Math.exp(regions[r].k * high);
            }
            
            if (f_high < target) {
                data.error = `La población nacional objetivo es extremadamente alta (${target.toLocaleString('es-ES')} hab.) y no se alcanzará en el horizonte de 200 años (máximo proyectado: ${Math.round(f_high).toLocaleString('es-ES')} hab. para el año ${2018 + Math.round(high)}).`;
            } else {
                let solved_t = 0;
                for (let i = 0; i < 80; i++) {
                    let mid = (low + high) / 2;
                ...
                data.steps = [
                    "1. Ecuación compuesta: P(t) = Σ P0_i * e^(k_i * t)",
                    "2. Método numérico de bisección local en el intervalo [0, 200] años.",
                    `3. Tiempo despejado obtenido: t ≈ ${solved_t.toFixed(4)} años.`,
                    `4. Año de coincidencia estimado: 2018 + ${solved_t.toFixed(1)} = ${solved_year}`
                ];
            }
        }
    } else {
        ...
        if (k < 0 && target > p0) {
            ...
            data.error = `La región tiene crecimiento nulo y su población permanecerá constante en ${p0.toLocaleString('es-ES')} hab.`;
        } else {
            ...
            data.solved_t = solved_t.toFixed(4);
            data.steps = [
                `1. Ecuación Malthusiana: P(t) = P0 * e^(k * t)`,
                `2. Despeje analítico de t: t = ln(P / P0) / k`,
                `3. Sustitución de valores: t = ln(${target.toLocaleString('es-ES')} / ${p0.toLocaleString('es-ES')}) / (${k})`,
                `4. Relación de escala P/P0 = ${ratio.toFixed(5)}`,
                `5. Tiempo despejado: t ≈ ${solved_t.toFixed(4)} años transcurridos`,
                `6. Año resultante = 2018 + ${solved_t.toFixed(2)} = ${solved_year}`
            ];
        }
    }
...
function displaySolverResult(data) {
    ...
    if (data.success) {
        ...
        document.getElementById('solver-err-title').textContent = "Cálculo Inviable";
...
// ==========================================================================
// 10.1 CALCULADORA DE POBLACIÓN FUTURA (COMPONENTE EXPLICITO NUEVO)
// ==========================================================================
function setupCalculatorForm() {
    const form = document.getElementById('calculator-form');
    const btn = document.getElementById('btn-calc');
    const spinner = btn.querySelector('.btn-spinner-calc');
    ...
    form.addEventListener('submit', (e) => {
        ...
        const regionKey = document.getElementById('calc-region').value;
        const targetYear = parseInt(document.getElementById('calc-year').value);

        if (spinner) spinner.classList.remove('hidden');
        btn.disabled = true;

        setTimeout(() => {
            calculateFuturePopulation(regionKey, targetYear);
...
function calculateFuturePopulation(regionKey, targetYear) {
    const resultBox = document.getElementById('calc-result-box');
    const resYearLabel = document.getElementById('calc-res-year-label');
    const resPopValue = document.getElementById('calc-res-pop-value');
    const resRegion = document.getElementById('calc-res-region');
    const resT = document.getElementById('calc-res-t');
    const formulaDisplay = document.getElementById('calc-formula-display');

    if (!resultBox) return;

    const t = targetYear - 2018;
    let popResult = 0;
    let mathSteps = '';

    if (regionKey === 'Total') {
        // La población total es la suma de cada región calculada para el tiempo t
        let sumPop = 0;
        let terms = [];
        for (let rKey in REGIONS_CONFIG) {
            const config = REGIONS_CONFIG[rKey];
            const p_r = config.p0 * Math.exp(config.k * t);
            sumPop += p_r;
            terms.push(`${config.name_es}: ${Math.round(p_r).toLocaleString('es-ES')} hab. (k = ${config.k})`);
        }
        popResult = Math.round(sumPop);
        resRegion.textContent = 'Tailandia (Total Nacional)';
        mathSteps = `P(${t}) = Σ P0_i * e^(k_i * t)\n\nCálculo para t = ${t} años transcurridos desde el 2018 (Año ${targetYear}):\n` + 
                    terms.map((tText, idx) => `  - [${idx+1}] ${tText}`).join('\n') + 
                    `\n\nSuma Total Proyectada = ${popResult.toLocaleString('es-ES')} hab.`;
    } else {
        const config = REGIONS_CONFIG[regionKey];
        popResult = Math.round(config.p0 * Math.exp(config.k * t));
        resRegion.textContent = config.name_es;
        mathSteps = `Fórmula: P(t) = P0 * e^(k * t)\n` +
                    `Sustitución para t = ${t} años transcurridos (Año ${targetYear}):\n` +
                    `  P(${t}) = ${config.p0.toLocaleString('es-ES')} * e^(${config.k} * ${t})\n` +
                    `  P(${t}) = ${config.p0.toLocaleString('es-ES')} * ${Math.exp(config.k * t).toFixed(6)}\n` +
                    `  P(${t}) ≈ ${popResult.toLocaleString('es-ES')} hab.`;
    }

    resYearLabel.textContent = targetYear;
    resPopValue.textContent = popResult.toLocaleString('es-ES') + ' hab.';
    resT.textContent = `${t} años`;
    formulaDisplay.textContent = mathSteps;
...
function performSearch(query) {
    ...
    if (popValue > 0) {
        ...
        // Generar el submit local
        solvePopulationLocal(document.getElementById('solver-region').value, popValue);
        return;
    }
